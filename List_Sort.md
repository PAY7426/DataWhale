...
一般情况下,元素 data[i] 的关键字 data[i].key由$d$位数字(或字符)组成,即 $k^{d-1}k^{d-2}\cdots k^2k^1$,每一个数字表示关键字的一位,其中$k^{d-1}$ 为最高位、$k^1$ 是最低位,每一位的值都在 $0<k^{i}<r$ 范围内,其中 $r$ 称为基数(radix)。例如,对于二进制数 $r$ 为 $2$,对于十进制数 $r$ 为 $10$。
